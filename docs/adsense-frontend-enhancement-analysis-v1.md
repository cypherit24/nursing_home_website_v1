...
## 8. MCP / Connector / AI 모델 사용 검토

> 작성일: 2026-04-01 | 검토 배경: 3가지 프론트엔드 강화 기능 구현 시 MCP, 외부 Connector, AI 모델 도입이 권장되는지 평가

### 8.1 MCP (Model Context Protocol) — 불필요

| 판정 항목 | 분석 |
|---|---|
| **필요성** | 3가지 기능 모두 외부 AI 서비스와 런타임 통신 불필요. 차트 데이터는 nursing_homes 테이블 컬럼에서 Supabase anon SELECT로 직접 조회 |
| **비용 충돌** | MCP 서버 호스팅 비용 발생 → plan-v3 §1 "$0 운영비용" 원칙 위반 |
| **아키텍처 적합성** | 정적 데이터 → 정적 렌더링 흐름에서 MCP 연동 레이어는 오버엔지니어링 |
| **결론** | **비권장** |

### 8.2 Connector (외부 미들웨어) — 기존 구조로 충분

| 판정 항목 | 분석 |
|---|---|
| **현재 구조** | Next.js App Router + Supabase Client가 이미 확정 (plan-v3) |
| **차트 데이터 흐름** | Server Component → Supabase `select()` → props → Client Component 차트 렌더링. 별도 API Route/Connector 불필요 |
| **갱신 주기** | ISR revalidate 30일 — 실시간 커넥터 필요 없음 |
| **결론** | **비권장**. 기존 Supabase Client로 충분 |

### 8.3 AI 모델 추가 사용 — 불필요, 역효과

| 판정 항목 | 분석 |
|---|---|
| **에드센스 전략 역행** | 본 방안의 핵심 = "AI가 아닌 팩트 기반 시각화"로 R11(AI 콘텐츠 페널티) 완화. AI 추가 사용은 "auto-generated content" 의심 재강화 |
| **비용** | 차트 라벨/해석을 AI 생성 시 2만 건 × API 호출 비용 발생. 현재 방안은 $0 |
| **스키마 영향** | AI 생성 텍스트 추가 시 seo_content JSON 스키마 변경 필요 → S5 동기화 이슈 촉발. 현재 방안은 스키마 변경 없음 |
| **보안** | AI 응답을 프론트엔드에 렌더링하면 S1 XSS 리스크 재도입 가능성 |
| **성능** | 런타임 AI 호출 시 TTFB 악화. 현재 SSG/ISR 전략 유지 불가 |
| **결론** | **비권장**. AI 의존도를 낮추는 것 자체가 본 방안의 핵심 가치 |

### 8.4 기대 효과 비교표

| 관점 | AI/MCP 미사용 (현 방안) | AI/MCP 사용 시 |
|---|---|---|
| 에드센스 승인 | 팩트 시각화 → "unique value" 증명 ✅ | AI 라벨/해석 → "auto-generated" 리스크 재발 ❌ |
| 비용 | $0 (MIT 라이선스 패키지만) | API + 호스팅 비용 발생 |
| 개발 복잡도 | T14~T16 내 +3~5시간 흡수 | 인프라 레이어 + 스키마 변경 + 테스트 대폭 증가 |
| 보안 (S1~S7) | 전체 통과 | S1 XSS, S5 스키마 동기화 리스크 재도입 |
| 성능 | SSG/ISR 유지 | TTFB 악화, 아키텍처 변경 필요 |
| plan-v3 호환 | 4개 태스크 범위 확장만 | 아키텍처 변경 → 승인 재진행 필요 |

### 8.5 최종 판정

**MCP, Connector, AI 모델 모두 비권장.**

3가지 프론트엔드 강화 기능은 **순수 프론트엔드(Tailwind + lucide-react + recharts) + DB 팩트 데이터** 조합으로 구현하는 것이 plan-v3의 비용 구조·보안 모델·에드센스 전략에 최적이다. AI 의존도를 높이는 것이 아니라 **낮추는 것**이 R11 리스크 완화의 본질이다.
