...
> 변경일: 2026-04-01 | adsense-frontend-enhancement-analysis-v1.md 기반 | 판정: 대기 (PM 검토 필요)

#### 핵심 변경: 에드센스 승인 리스크(R11) 해소를 위한 프론트엔드 시각적 강화

**변경 동기**: plan-v3 프론트엔드는 텍스트 중심 설계(SeoContent 템플릿 + 카드 리스트)로, 2만 페이지가 AI 생성 의혹 + thin content 판정을 받아 에드센스 승인이 거부될 리스크가 높다(R11: 발생 확률 중, 영향도 치명). 3가지 프론트엔드 강화(Skeleton UI, Badge/Icon, Interactive Chart)를 추가하여 "팩트 기반 시각화"로 고유 가치를 증명한다.

**설계 원칙**: MCP, 외부 Connector, AI 모델 추가 사용 없이 **순수 프론트엔드(Tailwind + lucide-react + recharts) + DB 팩트 데이터**로 구현. AI 의존도를 높이는 것이 아니라 낮추는 것이 R11 완화의 본질이다.

| 구분 | 변경 내용 | 근거 |
|---|---|---|
| 시각적 강화 A | **Skeleton UI 추가** (Tailwind animate-pulse 기반 loading.tsx) | ISR 19,000건 첫 방문 시 UX 보완. 추가 패키지 없음 |
| 시각적 강화 B | **Badge & Icon 추가** (lucide-react) | 사진 부재 보완, 시각적 밀도 대폭 향상 |
| 시각적 강화 C | **Interactive Chart 추가** (recharts, SVG 기반) | 공공 API 팩트 데이터 시각화로 "unique value" 증명. chart.js 금지 (S3 CSP 위반 위험) |
| 차트 데이터 소스 | nursing_homes 테이블 컬럼 직접 사용 | seo_content JSON 스키마 변경 불필요 (S5 영향 없음) |
| 비용 | **$0 추가** | lucide-react + recharts = MIT 라이선스 무료 |
| 보안 | S1~S7 **전체 통과** | recharts=SVG 렌더링 (S1 안전, S3 호환), 차트에 representative_name 비표시 (S4) |

#### 변경 태스크 목록

| 태스크 | 변경 요약 |
|---|---|
| T14 | `lucide-react`, `recharts` 패키지 추가 설치 |
| T15 | (1) 카드에 배지 아이콘 추가 (빈자리, 규모, 요양보호사 비율, 식비 수준), (2) `loading.tsx` 스켈레톤 추가 |
| T16 | (1) `loading.tsx` 스켈레톤, (2) 현황 카드에 아이콘 추가, (3) 비용/입소/인력 차트 컴포넌트 3개 추가, (4) CLS 방지 + null 처리 + SSR 가드 |
| T25 | 차트 렌더링 정상 확인 항목 추가 (20번째) |

#### 변경하지 않은 태스크

T01~T13-d, T17~T24, T26 — 프론트엔드 시각적 강화와 무관하거나 영향 없음
...
> v4 참고: 프론트엔드 강화(Skeleton/Badge/Chart)에 따른 추가 비용 $0. lucide-react + recharts = MIT 라이선스 무료. AI 재생성 불필요 (seo_content 스키마 변경 없음).
...
> v4: 변경 없음. 차트 데이터는 nursing_homes 테이블 컬럼에서 직접 조회하므로 스키마 영향 없음.
...
> v4 신규 섹션. 에드센스 R11 리스크 해소를 위한 3가지 강화 방안의 상세 사양을 정의한다.

### 4.1 차트 데이터 소스

차트에 사용할 숫자 데이터는 **seo_content JSON이 아닌 nursing_homes 테이블 컬럼**에서 직접 조회한다.

| 데이터 | DB 컬럼 | 타입 | 비고 |
|---|---|---|---|
| 정원 | capacity | 정수 | step2 API 수집 |
| 현원 | current_occupancy | 정수 | step2 API 수집 |
| 요양보호사 수 | caregiver_count | 정수 | step2 API 수집 |
| 식비/일 | meal_cost_per_day | 원 단위 | step2 API 수집 |
| 1인실 비용 | room_cost_1person | 원 단위 | step2 API 수집 |
| 2인실 비용 | room_cost_2person | 원 단위 | step2 API 수집 |

**핵심**: seo_content JSON 스키마 변경 불필요 → S5 동기화 이슈 없음, AI 재생성 불필요 (비용 $0)

### 4.2 Skeleton UI 사양

| 항목 | 사양 |
|---|---|
| 구현 방식 | Next.js App Router `loading.tsx` 파일 기반 Suspense |
| 스타일 | Tailwind CSS `animate-pulse` 클래스 |
| 적용 위치 | T15 카드 리스트 `loading.tsx`, T16 상세 페이지 `loading.tsx` |
| 추가 패키지 | 없음 |
| Server/Client | Server Component 유지 (`loading.tsx`는 RSC 지원) |
| CLS 영향 | CLS 0 유지 (스켈레톤이 실제 콘텐츠와 동일한 레이아웃 점유) |

### 4.3 Badge & Icon 사양

| 항목 | 사양 |
|---|---|
| 라이브러리 | `lucide-react` (MIT, tree-shakeable, RSC 호환, 아이콘 1,500+) |
| 대안 비교 | Heroicons (Tailwind 팀, RSC 지원) → 아이콘 수 300+, 요양원 도메인 특화 아이콘 부족 → lucide 채택 |
| Server/Client | Server Component에서 렌더링 가능 → 크롤러에 바로 노출 |

**배지 매핑**:

| 배지 | 아이콘 | 데이터 소스 | 로직 |
|---|---|---|---|
| 빈자리 | `BedDouble` | capacity - current_occupancy | > 0이면 표시 |
| 시설 규모 | `Building2` | capacity | 소(~30)/중(31~99)/대(100~) |
| 요양보호사 비율 | `HeartPulse` | caregiver_count / current_occupancy | 비율 표시 |
| 식비 수준 | `UtensilsCrossed` | meal_cost_per_day | 범위별 등급 |

### 4.4 Interactive Chart 사양

| 항목 | 사양 |
|---|---|
| 라이브러리 | `recharts` (MIT, SVG 기반, React 네이티브) |
| **사용 금지** | **chart.js** — Canvas 기반, 일부 플러그인 eval 사용 → S1 XSS + S3 CSP 위반 위험 |
| 번들 크기 | ~45KB gzipped. `"use client"` 코드 스플리팅으로 메인 페이지 번들 미포함 |
| SSR 가드 | `next/dynamic({ ssr: false })` — hydration mismatch 방지 |
| CLS 방지 | 차트 컨테이너에 고정 높이 `h-64` 지정 필수 |
| null 처리 | step2 미완료(detail_fetched_at IS NULL) 시설은 차트 비표시 + "데이터 없음" 안내 |
| 접근성 | 차트에 `aria-label` 추가. SVG `<desc>` 태그로 스크린리더 대체 텍스트 |
| S4 준수 | 차트에 representative_name 노출 절대 금지. 시설 통계 데이터만 시각화 |
...
**시도 평균 데이터**: 비용 비교 차트에 "시도 평균"을 표시하려면, 별도 RPC 함수 또는 빌드 시 집계 필요 → T16 구현 시 결정

### 4.5 LCP/CLS 영향 분석

| 기능 | LCP 영향 | CLS 영향 | 대응 |
|---|---|---|---|
| Skeleton UI | 체감 속도 향상 | CLS 0 (동일 레이아웃 점유) | 없음 |
| Badge/Icon | 무시할 수준 (SVG 고정 크기) | CLS 0 | 없음 |
| Chart | 클라이언트 렌더링 ~50KB 추가 | **시프트 가능** | `h-64` 고정 높이 + `next/dynamic` lazy loading |
...
| M4 | 프론트엔드 구축 | T14~T19 | Next.js 로컬 프리뷰에서 목록·상세 정상 작동. **스켈레톤·배지·차트 렌더링 정상** (T17 계산기, T18 Rate Limiting은 MVP 후 추가 가능) | 4주차 |
...
### T14: Next.js 15 프로젝트 초기 설정 ← v3 변경, v4 변경
- 마일스톤: M4
- 실행 환경: Codespaces
- 선행 조건: T04
- 작업 내용: Next.js 15 App Router 프로젝트 생성. Tailwind CSS 설정. @supabase/supabase-js 설치. **~~sanitize-html 설치~~** (v3 제거: JSON 템플릿 방식으로 불필요). **`lucide-react` 설치** (v4 추가: 배지/아이콘용, tree-shakeable, RSC 호환). **`recharts` 설치** (v4 추가: 차트용, SVG 기반, CSP 호환. **chart.js 사용 금지 — S3 위반 위험**). 환경변수 설정 (NEXT_PUBLIC_SUPABASE_URL, NEXT_PUBLIC_SUPABASE_ANON_KEY). **SeoContentJson TypeScript 인터페이스 생성** (v3 추가):
  ```typescript
...
- 완료 확인: `npm run dev` 정상 실행. localhost:3000에서 기본 페이지 표시. ESLint에서 dangerouslySetInnerHTML 사용 시 에러 발생 확인. **`lucide-react`, `recharts` package.json에 포함 확인**.
- 예상 소요: **1시간** (v3: 50분. v4: +10분 패키지 설치 및 확인)
- 비용: 없음
...
- 작업 내용:
  ...
  - 20건 페이지네이션
  - **v4 추가 — Skeleton UI**: `loading.tsx` 파일 생성. Tailwind `animate-pulse`로 카드 리스트 스켈레톤 구현. 실제 카드와 동일한 레이아웃으로 CLS 0 유지
  - **v4 추가 — Badge & Icon**: 카드에 lucide-react 아이콘 배지 4종 추가:
    - `BedDouble`: 빈자리 배지 (capacity - current_occupancy > 0)
    - `Building2`: 시설 규모 배지 (소/중/대)
    - `HeartPulse`: 요양보호사 비율 배지
    - `UtensilsCrossed`: 식비 수준 배지
...
- 완료 확인: 로컬 프리뷰에서 카드 20건 표시. 시도 필터 변경 시 결과 변경. 페이지네이션 작동. 폰트 18px 확인. 면책 고지 표시 확인. **스켈레톤 로딩 표시 확인 (네트워크 스로틀링). 4종 배지 아이콘 정상 렌더링 확인**.
- 예상 소요: **3시간** (v3: 2시간. v4: +1시간 스켈레톤/배지 구현)
- 비용: 없음

### T16: 상세 페이지 (/[facility_code]) 구현 ← v3 변경, v4 변경
- 마일스톤: M4
- 실행 환경: Codespaces
- 선행 조건: T15
- 작업 내용:
  ...
  - **v4 추가 — Skeleton UI**: `loading.tsx` 파일 생성. 상세 페이지 레이아웃(현황카드 + SEO 섹션 + 차트 영역)과 동일한 스켈레톤. Tailwind `animate-pulse`. CLS 0 유지
  - **v4 추가 — 현황 카드 아이콘**: 입소·인력·요양보호사 현황 카드에 lucide-react 아이콘 적용 (Server Component에서 렌더링 → 크롤러 노출)
  - **v4 추가 — Interactive Chart 3종**: `"use client"` 컴포넌트로 분리
    - **비용 비교 바 차트**: 식비·1인실·2인실 비용 (해당 시설 vs 시도 평균). recharts `<BarChart>`
    - **입소 현황 도넛 차트**: 현원 / 잔여 정원 (capacity 대비 점유율). recharts `<PieChart>`
    - **인력 비율 바 차트**: 요양보호사 수 / 입소자 수 (권장 비율 대비). recharts `<BarChart>`
    - **SSR 가드**: `next/dynamic({ ssr: false })` — hydration mismatch 방지
    - **CLS 방지**: 차트 컨테이너에 `className="h-64"` 고정 높이 지정
    - **null 처리**: `detail_fetched_at IS NULL` 시설은 차트 비표시 + "데이터 준비 중" 안내 문구. 조건부 렌더링: `{capacity != null && <ChartSection ... />}`
    - **접근성**: 각 차트에 `aria-label` 추가 + SVG `<desc>` 태그 스크린리더 대체 텍스트
    - **S4 준수**: 차트에 representative_name 절대 비표시. 시설 통계 데이터만 시각화
    - **비급여 면책 고지**: 비용 차트 하단에 "실제 금액과 다를 수 있습니다" 문구 포함
  - **시도 평균 데이터 조회**: 비용 비교 차트용. Server Component에서 시도별 평균 집계 쿼리 실행 → props로 전달. 구현 방식 선택 (RPC 함수 또는 빌드 시 집계) → T16 구현 시 결정
  - **접근성**: 시맨틱 HTML, 색상대비 WCAG AA 준수
- 완료 확인: 로컬 프리뷰에서 특정 기관 상세 페이지 정상 표시. JSON 기반 SEO 소개글의 6개 섹션 모두 렌더링 확인. 코드베이스에 dangerouslySetInnerHTML 미사용 확인 (`grep -r "dangerouslySetInnerHTML" src/` 결과 0건). **스켈레톤 로딩 표시 확인. 현황 카드 아이콘 렌더링 확인. 차트 3종 정상 렌더링 확인. 차트 null 데이터 시 "데이터 준비 중" 표시 확인. 차트 컨테이너 고정 높이(h-64) 확인. 차트 aria-label 존재 확인**.
- 예상 소요: **5시간** (v3: 2시간. v4: +3시간 스켈레톤/아이콘/차트 3종 구현)
- 비용: 없음
...
- 작업 내용: Vercel에 GitHub 리포지토리 연결. 환경변수 설정 (SUPABASE_URL, SUPABASE_ANON_KEY, KV 연결). Vercel KV 인스턴스 생성 및 연결 (T18 구현 시). 배포 실행. **빌드 시간 45분 이내 확인** (SSG 1,000건 기준 ~17분 예상). **v4 참고: recharts ~45KB + lucide 아이콘 ~5KB ≈ 50KB 번들 추가. 코드 스플리팅으로 메인 페이지 번들에 미포함. 빌드 시간 영향 무시할 수준 (+1~2분)**.
- 완료 확인: Vercel 대시보드에서 배포 성공. 빌드 시간 45분 이내. 배포 URL에서 메인·상세 정상 작동. **차트 렌더링 정상 확인**.
...
### T25: 최종 검증 체크리스트 실행 ← v3 변경, v4 변경
- 마일스톤: M6
- 실행 환경: 브라우저
- 선행 조건: T20
- 작업 내용: 아래 **22개** 항목 검증 (v3: 19개, v4: +3개):
  1. 메인 페이지 로드 정상
  2. 시도/시군구 필터 작동
  3. 페이지네이션 작동
  4. 상세 페이지 진입 정상
  ...
  20. **차트 3종(비용비교/입소현황/인력비율) 정상 렌더링 확인** (v4 추가)
  21. **차트 null 데이터 시 "데이터 준비 중" 안내 표시 확인** (v4 추가)
  22. **스켈레톤 로딩 정상 표시 확인 (메인 + 상세 페이지)** (v4 추가)
- 완료 확인: **22개** 항목 모두 통과
- 예상 소요: **1.5시간** (v3: 1시간. v4: +30분 차트/스켈레톤 검증)
- 비용: 없음
...
| R3 | 시설 사진 데이터 확보 불가 | 확정 | 중 | A안 채택 (홈페이지 URL 링크). **v4: 사진 부재를 lucide-react 아이콘 배지 + recharts 차트로 시각적 밀도 보완**. |
| R4 | adminPttnCd 폴백 로직 실패 | 낮 | 중 | A03→A01→A04→B03→C06 순서 폴백. 전체 실패 시 해당 기관 건너뛰기 + pipeline_errors 로깅. |
| R5 | 비급여 금액 단위 불일치 (일별/월별 혼재) | 중 | 낮 | 금액 그대로 저장 + 계산기에 면책 고지 강화. **v4: 비용 차트에도 "실제 금액과 다를 수 있습니다" 면책 고지 포함**. |
...
| **R11** | **구글 AI 생성 콘텐츠 페널티** | **중** | **치명** | **100건 파일럿 배포(T13-d) → 2주 색인 확인 → 전체 확장. 실제 데이터 수치 중심 콘텐츠 차별화. v4: Skeleton UI + Badge/Icon + Interactive Chart로 "팩트 기반 시각화" 고유 가치 증명 → thin content / auto-generated 의심 완화. 대안 수익모델 (시설 직접 마케팅, 네이버 유입).** |
| **R12** | **B550928 API 응답 구조 변경/중단** | **중** | **높음** | **최초 응답 스키마 저장(schemas/). 구조 변경 감지→GitHub Issue. 그레이스풀 디그레이드(xlsx 기본 데이터로 렌더링). DB 영구 캐싱.** |
| **R13** | **개인정보보호법 위반 리스크** | **낮** | **높음** | **공공데이터법 3조 근거. 대표자 성명 비표시(DB만 보관). 기관 대표번호만 표시. 면책 고지. 삭제 요청 24시간 처리. v4: 차트에도 S4 적용 — representative_name 절대 비표시.** |
...
> v4 참고: recharts는 SVG 기반 렌더링으로 dangerouslySetInnerHTML을 사용하지 않으며, CSP `script-src 'self'`와 완전 호환. lucide-react도 SVG React 컴포넌트로 JSX 자동 이스케이핑 적용. **S1/S3 위반 없음.**
...
- **v4 추가**: 차트(비용비교/입소현황/인력비율)에도 S4 적용 — representative_name 절대 비표시. 시설 통계 숫자 데이터만 시각화
...
- **v4 추가**: 차트에 `aria-label` 추가 + SVG `<desc>` 태그로 스크린리더용 대체 텍스트 제공. 차트 색상도 WCAG AA 대비 4.5:1 준수
...
4주차 ─────────────────────────────────────────────
  T14        Next.js 초기 설정 (SeoContentJson 타입 + ESLint no-danger + lucide-react + recharts)
  T15        메인 페이지 (접근성 + 면책 고지 + 대표자명 비표시 + 스켈레톤 + 배지/아이콘)
  T16        상세 페이지 (SSG 1,000건 + ISR + SeoContent 템플릿 + 스켈레톤 + 아이콘 + 차트 3종)
...
5주차 ─────────────────────────────────────────────
  T20        Vercel 배포 (빌드 시간 확인, ~17분 예상, +50KB 번들)
  T21        Deploy Hook
  T22        스케줄 워크플로우 완성
  T23        도메인 연결 (선택)
...
6주차 (검증 + 버퍼) ──────────────────────────────
  ...
  T25        최종 검증 체크리스트 (22개 항목, 차트/스켈레톤 검증 포함)
  T26        모니터링 + 수익화 준비
...
### M4: 프론트엔드
- [ ] T14: Next.js 15 프로젝트 초기 설정 (SeoContentJson 타입 + ESLint no-danger + lucide-react + recharts)
- [ ] T15: 메인 페이지 구현 (접근성 + 면책 고지 + 스켈레톤 + 배지/아이콘)
- [ ] T16: 상세 페이지 구현 (SSG 1,000건 + ISR + SeoContent 템플릿 + 스켈레톤 + 아이콘 + 차트 3종)
- [ ] T17: 자부담금 계산기 구현 (MVP 후 추가 가능)
- [ ] T18: Rate Limiting 미들웨어 구현 (MVP 후 추가 가능)
- [ ] T19: SEO 메타데이터 + sitemap.xml + CSP 헤더

### M5: 배포 및 자동화
- [ ] T20: Vercel 배포 (차트 렌더링 포함 확인)
...
- [ ] T25: 최종 검증 체크리스트 실행 (22개 항목, 차트/스켈레톤 검증 포함)
- [ ] T26: 모니터링 및 수익화 준비
- [ ] 파일럿 100건 색인 최종 확인
