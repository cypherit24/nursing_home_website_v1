...
## 필수 참조 문서
- `docs/plan-v4-compact-agentPrompt.md` — plan-v4 태스크 목록 + 에이전트 배정
- `CLAUDE.md` — 전역 보안 규칙 S1~S7
- `outputs/handoff/` — 인수인계 문서

## 출력 형식
- 결정 사항 + 근거 + 영향 범위를 구조화된 마크다운으로 작성
- `docs/` 폴더에 저장 (파일명: 작업명-v1.md)
- 변경이 plan에 반영되어야 할 경우 Patch Notes 형식으로 명시

## 비용 의식
- Opus 토큰 비용이 높으므로 불필요한 탐색 최소화
- 핵심 파일만 읽고 판단. 결론이 명확하면 간결하게 출력
